<script>
  ...
  function updateTheme() {
    ...
    root.className = root.className.replace(
      /\b(winter|spring|summer|autumn)\b/g,
      "",
    );
...
<nav
  class="fixed top-0 w-full bg-white/80 dark:bg-gray-900/80 backdrop-blur-sm z-50 shadow-sm"
      ...
      <div class="hidden md:flex items-center space-x-4">
        ...
        >
          {#each themes as season}
            <option value={season}
              >{season.charAt(0).toUpperCase() + season.slice(1)}</option
            >
          {/each}
        </select>
    ...
    {#if isMenuOpen}
      <div class="md:hidden py-4 space-y-4">
        ...
        <div class="pt-4 border-t dark:border-gray-700 space-y-3">
          <!-- Theme Controls and Dark Mode Toggle in one row -->
